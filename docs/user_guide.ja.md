...
> ⚠ 英語版より古い (as of 2026-06-24)。英語版が正です（CLI `psmpy run` /
> `--report`、§5 レポーティングの model-1 経路 `from_solution`/`from_run_dir`/
> `write_iamc` は英語版で更新済み）。日本語版は別途まとめて翻訳します。
...
> 実 Japan ラン は HPC ゲート待ち（`docs/STATUS.md`）。**統一的な solve CLI は
> まだ無く**、求解は下記の Python API で駆動します。
...
uv run pytest tests/    # テスト（solverテストは GAMS ライセンス無しで自動スキップ）
```
...
求解結果は reporting アダプタ経由で Track P **Report スキーマ**フレームへ変換
されます（純 pandas の Track P コアを再利用 — gamspy 不要）。`to_report` は
3つの入力 — `RunResult`・**IR**（バス decode＋地域階層用）・コンパイル済み
**`SymbolFrames`** — を必要とするので、`sf` がスコープにある明示経路を使います：
...
frames = to_report(result, model._ir(), sf)   # RunResult + IR + SymbolFrames -> フレーム
write_report(frames, "reports/toy")            # parquet（tools/pyreport が消費）
```

`model._ir()` が現状の IR アクセサ（公開 reporting ファサードの追加は将来の自然な
改善）。正確なシグネチャは `src/psmpy/reporting/adapter.py`、数量カタログと IAMC
出力は `docs/spec/postprocess.md` を参照。`tools/pyreport/`
（`PowerSystemModel-master` 内）がこれらのフレームを表/IAMC/プロットに変換します。
...
| コマンド | 用途 |
| --- | --- |
| `uv run pytest tests/` | 全テスト（solverテストはライセンス無しで自動スキップ） |
| `uv run python -m psmpy.units --dump [--out PATH]` | 正準 unit-token 契約を出力（JSON） |
| `uv run --extra solve python -m psmpy.extract.run_extract --gdx GDX --out DIR` | GDX → 正準データセット |

**`psmpy solve` CLI はまだありません** — 求解は Python から（§4）。5カテゴリの
シナリオ設定＋オーバーレイ（M3）は API として存在（`docs/spec/scenario.md`）。
CLI 化は今後の課題です。
